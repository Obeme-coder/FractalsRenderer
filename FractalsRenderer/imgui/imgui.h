...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_AlphaBar        = 1 << 16,  //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient_texture in picker.
...
struct ImGuiIO
{
    ...
    ImVec2      DisplayFramebufferScale;        // = (1, 1)         // For retina display or other situations where window coordinates are different from msaa_compute_framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.
...
struct ImDrawData
{
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and msaa_compute_framebuffer resolution.
};
